...
class UserAuthSourceImpl extends UserAuthSource {
  ...
  Future<bool> isUserExists(UserAuthDataModel userAuth) async {
    var querySnapShot = await usersCollection
        .where(UserAuthDataModel.emailCol, isEqualTo: userAuth.email)
        .get();
    if(querySnapShot.docs.isEmpty){
      throw UserNotFoundException();
    }

    var authData = UserAuthDataModel.fromMap(querySnapShot.docs.first.data());
    if(authData.password != userAuth.password){
  ...
  @override
  Future<void> createAccount(UserAuthDataModel userAuth) async {
    var querySnapShot = await usersCollection
        .where(UserAuthDataModel.emailCol, isEqualTo: userAuth.email)
        .get();
    if (querySnapShot.docs.isEmpty) {
  ...
  Future<String?> _getDocumentId(String email) async {
    var querySnapShot = await usersCollection
        .where(UserAuthDataModel.emailCol, isEqualTo: email)
        .get();
