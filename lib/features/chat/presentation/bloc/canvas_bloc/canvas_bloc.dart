...
import 'package:ai_chat_bot/features/chat/domain/models/custom_line.dart';
import 'package:ai_chat_bot/features/chat/domain/models/quadrilateral.dart';
import 'package:ai_chat_bot/features/chat/domain/models/triangle.dart';
...
class CanvasBloc extends Bloc<CanvasEvent, CanvasState> {
  ...
  FutureOr<void> _handleStartEvent(CanvasStartEvent event, Emitter<CanvasState> emit) {
  ...
  FutureOr<void> _handleUpdateEvent(CanvasUpdateEvent event, Emitter<CanvasState> emit) {
  ...
  FutureOr<void> _handleEndEvent(CanvasEndEvent event, Emitter<CanvasState> emit) {
  ...
  FutureOr<void> _handleUndoEvent(CanvasUndoEvent event, Emitter<CanvasState> emit) {
  ...
  FutureOr<void> _handleRedoEvent(CanvasRedoEvent event, Emitter<CanvasState> emit) {
  ...
  FutureOr<void> _handleInitialEvent(CanvasInitialEvent event, Emitter<CanvasState> emit) {
    trashStack = [];
    previous = [];
    begin = Offset.zero;
    end = Offset.zero;
    emit(CanvasUpdatedState(current: Line(begin: begin, end: end) , track: const[]));
  }
